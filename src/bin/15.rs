...
pub fn part_two(input: &str) -> Solution {
    ...
    for row in (0..=UPPER_LIMIT).rev() {
        ...
        for p in &pairs {
            ...
            let diff = max_dist - row_dist;
            if diff > 0 {
                let start = max(p.sensor.x - diff, LOWER_LIMIT);
                let end = min(p.sensor.x + diff, UPPER_LIMIT);
...
mod tests {
    ...
    fn test() {
        let input = advent_of_code::read_file("inputs", 15);
        assert_eq!(part_one(&input), Solution::U32(5299855));
        assert_eq!(part_two(&input), Solution::U64(13615843289729));
    }
}
